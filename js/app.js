...
// Full-season opponent schedule, each team's 18 weeks: "@OPP" | "OPP" | "BYE".
const DEFAULT_NFL_SCHEDULE = {
...
// Fills in any team row not already set — never overwrites a row you've
// corrected or pasted in yourself.
function applyDefaultNflSchedule(state) {
  NFL_TEAMS.forEach((team) => {
    if (!state.nflSchedule[team] && DEFAULT_NFL_SCHEDULE[team]) {
      state.nflSchedule[team] = DEFAULT_NFL_SCHEDULE[team].slice();
    }
  });
}
...
function load() {
  try {
    ...
    if (raw) {
      ...
      // Fills in any NFL team's schedule row that isn't already set (e.g. one
      // you haven't corrected yet) — safe to run every load since it never
      // overwrites a row that's already present.
      applyDefaultNflSchedule(parsed);
      return parsed;
    }
...
document.getElementById("nflScheduleImportBtn").addEventListener("click", () => {
  ...
  const problems = [];
  lines.forEach((line) => {
    // Accepts comma-, tab-, or space-separated rows (e.g. a paste from a spreadsheet).
    const parts = line.split(/[\s,]+/).map((s) => s.trim()).filter((s) => s !== "");
    const team = (parts[0] || "").toUpperCase();
    if (team === "TEAM") return; // header row
